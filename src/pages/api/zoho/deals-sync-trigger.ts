...
export const POST: APIRoute = async () => {
  ...
  if (!instanceId) return customJson({ error: 'Not authenticated' }, { status: 401 });

  const supabase = getSupabase();

  // Return early if a deals sync is already running
  ...
  if (active && (active.stats as any)?.jobType === 'deals') {
    return customJson({ jobId: active.id, status: active.status, alreadyRunning: true });
  }

  // phase1_done:true skips Wix→Zoho direction — deals only sync Zoho→Supabase
