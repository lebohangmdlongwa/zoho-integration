...
async function refreshTokenIfNeeded(ctx: ZohoContext): Promise<void> {
  ...
  const params = new URLSearchParams({
    ...
    client_secret: (import.meta.env.ZOHO_CLIENT_SECRET ?? process.env.ZOHO_CLIENT_SECRET)!,
...
export async function zohoFetch<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`ZOHO ${init.method ?? 'GET'} ${path} → ${res.status}: ${body}`);
  }
...
export async function createZohoContext(instanceId: string): Promise<ZohoContext> {
  const token = await getToken(instanceId);
  if (!token) throw new Error(`ZOHO not connected for instance: ${instanceId}`);
  if (!token.api_domain) throw new Error(`ZOHO api_domain missing for instance: ${instanceId}`);
...
export async function searchContactByEmail(
  ...
  try {
    ...
    if (String(err).includes('204') || String(err).includes('no data')) return null;
    throw err;
  }
}
...
export async function searchContactByWixId(
  ...
  try {
    const encoded = encodeURIComponent(`(Wix_Contact_Id:equals:${wixContactId})`);
    const result = await zohoFetch<ZohoListResponse<ZohoContact>>(
      ctx,
      `/Contacts/search?criteria=${encoded}`,
    );
...
export async function getContact(
  ...
  try {
    const result = await zohoFetch<ZohoListResponse<ZohoContact>>(ctx, `/Contacts/${id}`);
    return result.data?.[0] ?? null;
  } catch (err) {
...
export async function createContact(
  ...
  const result = await zohoFetch<{ data: ZohoCreateResult[] }>(ctx, '/Contacts', {
    method: 'POST',
...
export async function updateContact(
  ...
  const result = await zohoFetch<{ data: ZohoCreateResult[] }>(ctx, `/Contacts/${id}`, {
    method: 'PUT',
...
export async function deleteContact(ctx: ZohoContext, id: string): Promise<void> {
...
export async function listContactFields(
  ...
  const result = await zohoFetch<ZohoFieldsResponse>(ctx, '/settings/fields?module=Contacts');
...
export async function zohoIntegrationFieldsExist(ctx: ZohoContext): Promise<boolean> {
...
export async function ensureCustomField(
  ...
  const exists = fields.some(
    (f) => f.field_label === fieldLabel || f.api_name === fieldLabel.replace(/ /g, '_'),
  );
...
export async function ensureIntegrationFields(ctx: ZohoContext): Promise<void> {
  ...
  for (const label of fields) {
    await ensureCustomField(ctx, label).catch((err) =>
      logger.warn('[zoho] ensureCustomField failed', { label, err: String(err) }),
    );
  }
}
...
export async function registerWebhookChannel(
  ...
  const actualExpiry = entry?.details?.events?.[0]?.channel_expiry ?? requestedExpiry;
  ...
  if (entry?.status === 'error' || (entry?.code && entry.code !== 'SUCCESS')) {
    throw new Error(`ZOHO channel registration failed: ${entry.code} – ${entry.message}`);
  }
...
export async function renewWebhookChannel(
  ...
  const result = await zohoFetch<{
    watch: Array<{ code?: string; status?: string; details?: { events?: Array<{ channel_expiry?: string }> } }>;
  }>(ctx, '/actions/watch', {
    method: 'PATCH',
    body: JSON.stringify({
...
export async function deleteWebhookChannel(ctx: ZohoContext, channelId: string): Promise<void> {
  const result = await zohoFetch<{
    watch: Array<{ code?: string; status?: string; message?: string }>;
  }>(ctx, `/actions/watch?channel_ids=${channelId}`, { method: 'DELETE' });

  const entry = result?.watch?.[0];
  logger.info('[zoho] deleteWebhookChannel', { channelId, code: entry?.code, status: entry?.status });
}
